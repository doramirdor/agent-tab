...
export function renderReceipt(r: RunReport): string {
  ...
  L.push("");
  L.push(c.bold(c.cyan("  OpenBar") + c.dim("  ·  " + toolLabel(r.tool))));
  L.push("");
  ...
  if (headline) {
    ...
    L.push(c.dim(`  run openbar fix  to turn the biggest wastes into agent rules`));
  } else {
...
export function renderReceiptPlain(r: RunReport): string {
  const prev = process.env.OPENBAR_NO_COLOR;
  process.env.OPENBAR_NO_COLOR = "1";
  try {
    return renderReceipt(r);
  } finally {
    if (prev === undefined) delete process.env.OPENBAR_NO_COLOR;
    else process.env.OPENBAR_NO_COLOR = prev;
  }
}
