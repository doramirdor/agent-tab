// `agent-tab fix` — turn detected waste into agent rules (CLAUDE.md / AGENTS.md).
...
const START = "<!-- agent-tab:start -->";
const END = "<!-- agent-tab:end -->";
...
function buildBlock(rules: string[], report: RunReport): string {
  ...
  lines.push("");
  lines.push(
    `<!-- Generated by agent-tab from session ${report.sessionId.slice(0, 8)} ` +
      `(bloat ${report.bloatScore}/100). Re-run \`agent-tab fix\` to refresh. -->`,
  );
  lines.push("");
