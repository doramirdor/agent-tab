// `openbar summary` — a local aggregate of recent runs (your weekly agent bill).
...
export function runSummary(argv: string[]): number {
  ...
  if (!s || s.runs === 0) {
    process.stdout.write(
      c.dim(
        all
          ? "No saved runs yet. Run  openbar report  after a session.\n"
          : `No runs in the last ${days} days. Try  openbar summary --all\n`,
      ),
    );
    return 0;
  }
  ...
  const L: string[] = [];
  L.push("");
  L.push(c.bold(c.cyan("  OpenBar") + c.dim(`  ·  bill (${period})`)));
  L.push("");
  ...
  L.push("");
  L.push(c.dim("  openbar fix  bakes the recurring ones into your agent rules"));
  L.push("");
