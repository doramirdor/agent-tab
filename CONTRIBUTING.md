...
Thanks for helping. OpenBar is a small, dependency-light TypeScript CLI, so the loop
is fast.

## Setup

```bash
git clone https://github.com/doramirdor/openbar
cd openbar
...
To see what the collector is doing, set `OPENBAR_DEBUG=1` — it appends to
`.openbar/openbar.log`.
