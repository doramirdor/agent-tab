...
[private vulnerability reporting](https://github.com/doramirdor/bartab/security/advisories/new)
...
BarTab is local-first and runs on your machine:
...
- It **writes** only inside `.bartab/` in your project, your agent settings files
...
- `install` registers a hook that runs `bartab hook` at your agent's lifecycle points.
  The command is an absolute path to this package's launcher; inspect it any time with
  `bartab install --print`.
- Event logs under `.bartab/runs/` may contain prompt text and file paths from your
  sessions. `.bartab/` is added to `.gitignore` by `install`; keep it out of commits.
- Codex skips freshly written command hooks until you explicitly trust them (`/hooks`),
  which is an intentional safety boundary — BarTab does not bypass it for you.
